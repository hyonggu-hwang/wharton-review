...
::: {.column-margin}
Repeated mistake: I kept thinking the left hand side as the value function *in the next economic period*. This is not right!!! Each iteration represents an *improved guess* of the same stationary value function. We are trying to arrive at the best approximation of the same object $V^*$, not find the next-period value function. In fact, we want to find the point where current and next period value functions are numerically equal, i.e. when $V$ is stationary. 
:::
