...
::: {.column-margin}
One thing I missed when coding this up for the first time was to distinguish the grid for state $K$ and policy $K'$.
:::
...
We already computed $E(\cdot)$ in the previous step, so now we compute the expectation term . For each current states $(K,z)$ and each candidate choice of $K'$, we take the expectation of the continuation value $V(K',z')$ over all possible next-period shocks $z'$, conditional on current $z$. Combining this expectation with the current-period return gives us the Bellman operator applied to our current guess of the value function. Iterating on this operator produces the **next approximation** to the fixed point $V^*(K,z)$. 

::: {.column-margin}
Repeated mistake: I kept thinking the LHS as the value function in the next *economic period*. This is incorrect! Each iteration represents an *improved guess* of the same stationary value function. We want the best approximation of $V^*$, not the next-period value function. That is, we want the point where current and next period value functions are numerically equal, i.e. when $V$ is stationary. 
:::

Recall that $V$ is a function of two state variables, $z$ and $K$, and we represent a discretized version of this function as a matrix. Each entry of this matrix is the value of being in each state $(z, K) pair $. To compute the expectation term, we use transition matrix `trans`, paying careful attention to the dimensions. The transition matrix is of size `N_z x N_z` and value function is a `N_z x N_k` matrix. The resulting matrix multiplication produces a `N_z x N_k` matrix, where each entry is the expected continuation value for each $(z', K')$ pair.
