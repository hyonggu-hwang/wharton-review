...
Using notation from summer school notes, we can write the Bellman equation as:

::: {.column-margin}
The lecture notes uses a different set of notations, where $E$ is $R$, $K$ is $s$, and $z$ is $x$.
:::
$$ V(K,z) = \max_{K' \in \Gamma(K,z)} \left[ R(K,z,K') + \beta \, \mathbb{E}_{z'|z} \, V(K',z') \right] $$
...
When calculating this difference between the current and previous value functions, we will use the supremum norm as a measure of distance (which is a standard practice). That is, if the maximum absolute difference is less than the tolerance level, we stop iterating. The following code implements this logic.
